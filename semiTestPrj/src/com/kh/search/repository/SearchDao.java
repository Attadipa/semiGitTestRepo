...
public class SearchDao {
	...
	public List<TradeVo> searchToKeyword(Connection conn, String keywords, PageVo pageVo) {
		...
		String sql = "SELECT * FROM ( SELECT ROWNUM RNUM, T.* FROM (SELECT T.TRADE_NO, M.MEMBER_MID WRITER, T.TITLE, EXPLAIN, TO_CHAR(T.ENROLL_DATE, 'YY/MM/DD HH:MI') ENROLL_DATE FROM TRADE T JOIN MEMBER M ON T.WRITER = M.MEMBER_NO ORDER BY TRADE_NO DESC) T ) WHERE "+ keywords.trim() +" AND RNUM BETWEEN ? AND ?";
