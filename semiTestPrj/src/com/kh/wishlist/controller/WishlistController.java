...
public class WishlistController extends HttpServlet{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//listCount 값 구하기
		listCount = new WishlistService().getCountForMy(memberNo);
		...
		List<WishVo> voList = new WishlistService().list(memberNo, pageVo);
		
		if(voList != null) {
			//조회 성공 -> tradeList 화면 보여주기
			req.setAttribute("pageVo", pageVo);
			req.setAttribute("voList", voList);
			req.getRequestDispatcher("/views/wishList/wishList.jsp").forward(req, resp);
		} else {
			//실패
